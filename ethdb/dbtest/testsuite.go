...
func TestDatabaseSuite(t *testing.T, New func() ethdb.KeyValueStore) {
		...
		"Iterator", func(t *testing.T) {
			...
			for i, tt := range tests {
				...
				for it.Next() {
					if len(tt.order) <= idx {
						t.Errorf(
							"test %d: prefix=%q more items than expected: checking idx=%d (key %q), expecting len=%d",
					...
					if !bytes.Equal(it.Key(), []byte(tt.order[idx])) {
						t.Errorf(
							"test %d: item %d: key mismatch: have %s, want %s", i, idx, string(it.Key()), tt.order[idx],
						)
					}
					if !bytes.Equal(it.Value(), []byte(tt.content[tt.order[idx]])) {
						t.Errorf(
							"test %d: item %d: value mismatch: have %s, want %s", i, idx, string(it.Value()),
							tt.content[tt.order[idx]],
						)
					}
					idx++
				}
	...
	t.Run(
		"KeyValueOperations", func(t *testing.T) {
			db := New()
			defer db.Close()
	...
	t.Run(
		"OperationsAfterClose", func(t *testing.T) {
			db := New()
...
func BenchDatabaseSuite(b *testing.B, New func() ethdb.KeyValueStore) {
	...
	b.Run(
		"Write", func(b *testing.B) {
			...
			b.Run(
				"WriteSorted", func(b *testing.B) {
					benchWrite(b, sKeys, sVals)
				},
			)
			b.Run(
				"WriteRandom", func(b *testing.B) {
	...
	b.Run(
		"Read", func(b *testing.B) {
			...
			b.Run(
				"ReadSorted", func(b *testing.B) {
					benchRead(b, sKeys, sVals)
				},
			)
			b.Run(
				"ReadRandom", func(b *testing.B) {
	...
	b.Run(
		"Iteration", func(b *testing.B) {
			...
			b.Run(
				"IterationSorted", func(b *testing.B) {
					benchIteration(b, sKeys, sVals)
				},
			)
			b.Run(
				"IterationRandom", func(b *testing.B) {
	...
	b.Run(
		"BatchWrite", func(b *testing.B) {
			...
			b.Run(
				"BenchWriteSorted", func(b *testing.B) {
					benchBatchWrite(b, sKeys, sVals)
				},
			)
			b.Run(
				"BenchWriteRandom", func(b *testing.B) {

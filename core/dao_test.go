...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	genDb, prefix, _ := GenerateChainWithGenesis(
		gspec, ethash.NewFaker(), int(forkBlock.Int64()-1), func(i int, gen *BlockGen) {},
	)
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		blocks, _ = GenerateChain(
			&proConf, conBc.GetBlockByHash(conBc.CurrentBlock().Hash()), ethash.NewFaker(), genDb, 1,
			func(i int, gen *BlockGen) {},
		)
		...
		blocks, _ = GenerateChain(
			&conConf, conBc.GetBlockByHash(conBc.CurrentBlock().Hash()), ethash.NewFaker(), genDb, 1,
			func(i int, gen *BlockGen) {},
		)
		if _, err := conBc.InsertChain(blocks); err != nil {
		...
		bc.Stop()
		blocks, _ = GenerateChain(
			&conConf, proBc.GetBlockByHash(proBc.CurrentBlock().Hash()), ethash.NewFaker(), genDb, 1,
			func(i int, gen *BlockGen) {},
		)
		...
		blocks, _ = GenerateChain(
			&proConf, proBc.GetBlockByHash(proBc.CurrentBlock().Hash()), ethash.NewFaker(), genDb, 1,
			func(i int, gen *BlockGen) {},
		)
		if _, err := proBc.InsertChain(blocks); err != nil {

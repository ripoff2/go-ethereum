...
import (
	...
	cmath "github.com/ripoff2/go-ethereum/common/math"
...
func IntrinsicGas(
	data []byte, accessList types.AccessList, isContractCreation, isHomestead, isEIP2028, isEIP3860 bool,
) (uint64, error) {
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	var (
		...
		rules  = st.evm.ChainConfig().Rules(
			st.evm.Context.BlockNumber, st.evm.Context.Random != nil, st.evm.Context.Time,
		)
	...
	gas, err := IntrinsicGas(
		msg.Data, msg.AccessList, contractCreation, rules.IsHomestead, rules.IsIstanbul, rules.IsShanghai,
	)
	if err != nil {
		return nil, err
	}
	...
	if rules.IsShanghai && contractCreation && len(msg.Data) > params.MaxInitCodeSize {
		return nil, fmt.Errorf(
			"%w: code size %v limit %v", ErrMaxInitCodeSizeExceeded, len(msg.Data), params.MaxInitCodeSize,
		)
	}

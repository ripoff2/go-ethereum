...
func testGeneration(t *testing.T, scheme string) {
	...
	stRoot := helper.makeStorageTrie(
		common.Hash{}, []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, false,
	)

	helper.addTrieAccount(
		"acc-1", &types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
	)
	helper.addTrieAccount(
		"acc-2", &types.StateAccount{
			Balance: uint256.NewInt(2), Root: types.EmptyRootHash, CodeHash: types.EmptyCodeHash.Bytes(),
		},
	)
	helper.addTrieAccount(
		"acc-3", &types.StateAccount{Balance: uint256.NewInt(3), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
	)

	helper.makeStorageTrie(
		hashData([]byte("acc-1")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
	)
	helper.makeStorageTrie(
		hashData([]byte("acc-3")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
	)
...
func checkSnapRoot(t *testing.T, snap *diskLayer, trieRoot common.Hash) {
	...
	snapRoot, err := generateTrieRoot(
		nil, "", accIt, common.Hash{}, stackTrieGenerate,
...
func testGenerateExistentStateWithWrongStorage(t *testing.T, scheme string) {
	...
	helper.addAccount(
		"acc-1", &types.StateAccount{
			Balance: uint256.NewInt(1), Root: types.EmptyRootHash, CodeHash: types.EmptyCodeHash.Bytes(),
		},
	)
	helper.addSnapStorage("acc-1", []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"})
	...
	stRoot := helper.makeStorageTrie(
		hashData([]byte("acc-2")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
	)
	helper.addAccount(
		"acc-2", &types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
	)
	...
	// Miss slots
	{
		...
		helper.makeStorageTrie(
			hashData([]byte("acc-4")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
		)
		helper.addAccount(
			"acc-4",
			&types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
		...
		helper.makeStorageTrie(
			hashData([]byte("acc-5")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
		)
		helper.addAccount(
			"acc-5",
			&types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
	...
	// Wrong storage slots
	{
		...
		helper.makeStorageTrie(
			hashData([]byte("acc-6")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
		)
		helper.addAccount(
			"acc-6",
			&types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
		...
		helper.makeStorageTrie(
			hashData([]byte("acc-7")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
		)
		helper.addAccount(
			"acc-7",
			&types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
		...
		helper.makeStorageTrie(
			hashData([]byte("acc-8")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
		)
		helper.addAccount(
			"acc-8",
			&types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
		...
		helper.makeStorageTrie(
			hashData([]byte("acc-9")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
		)
		helper.addAccount(
			"acc-9",
			&types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
	...
	// Extra storage slots
	{
		...
		helper.makeStorageTrie(
			hashData([]byte("acc-10")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
		)
		helper.addAccount(
			"acc-10",
			&types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
		...
		helper.addSnapStorage(
			"acc-10", []string{"key-0", "key-1", "key-2", "key-3"}, []string{"val-0", "val-1", "val-2", "val-3"},
		)

		// Account 11, non empty root but extra slots in the middle
		helper.makeStorageTrie(
			hashData([]byte("acc-11")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
		)
		helper.addAccount(
			"acc-11",
			&types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
		...
		helper.addSnapStorage(
			"acc-11", []string{"key-1", "key-2", "key-2-1", "key-3"}, []string{"val-1", "val-2", "val-2-1", "val-3"},
		)

		// Account 12, non empty root but extra slots in the end
		helper.makeStorageTrie(
			hashData([]byte("acc-12")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
		)
		helper.addAccount(
			"acc-12",
			&types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
		...
		helper.addSnapStorage(
			"acc-12", []string{"key-1", "key-2", "key-3", "key-4"}, []string{"val-1", "val-2", "val-3", "val-4"},
		)
	}
...
func testGenerateExistentStateWithWrongAccounts(t *testing.T, scheme string) {
	...
	// Wrong accounts
	{
		...
		helper.addSnapAccount(
			"acc-2", &types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: common.Hex2Bytes("0x1234")},
		)
	...
	// Extra accounts, only in the snap
	{
		...
		helper.addSnapAccount(
			"acc-5", &types.StateAccount{
				Balance: uint256.NewInt(1), Root: types.EmptyRootHash, CodeHash: common.Hex2Bytes("0x1234"),
			},
		) // Middle
		helper.addSnapAccount(
...
func testGenerateCorruptAccountTrie(t *testing.T, scheme string) {
	...
	) // 0xc7a30f39aff471c95d8a837497ad0e49b65be475cc0953540f80cfcdbdcd9074
	helper.addTrieAccount(
		"acc-2", &types.StateAccount{
			Balance: uint256.NewInt(2), Root: types.EmptyRootHash, CodeHash: types.EmptyCodeHash.Bytes(),
		},
	) // 0x65145f923027566669a1ae5ccac66f945b55ff6eaeb17d2ea8e048b7d381f2d7
	helper.addTrieAccount(
		"acc-3", &types.StateAccount{
			Balance: uint256.NewInt(3), Root: types.EmptyRootHash, CodeHash: types.EmptyCodeHash.Bytes(),
		},
	) // 0x19ead688e907b0fab07176120dceec244a72aff2f0aa51e8b827584e378772f4
...
func testGenerateMissingStorageTrie(t *testing.T, scheme string) {
	...
	stRoot := helper.makeStorageTrie(
		hashData([]byte("acc-1")), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true,
	) // 0xddefcd9376dd029653ef384bd2f0a126bb755fe84fdcc9e7cf421ba454f2bc67
	helper.addTrieAccount(
		"acc-1", &types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
...
func testGenerateWithExtraAccounts(t *testing.T, scheme string) {
	...
	if data := rawdb.ReadStorageSnapshot(
		helper.diskdb, hashData([]byte("acc-2")), hashData([]byte("b-key-1")),
	); data == nil {
	...
	}
	{
		...
		for i := 0; i < 1000; i++ {
			acc := &types.StateAccount{
				Balance: uint256.NewInt(uint64(i)), Root: types.EmptyRootHash, CodeHash: types.EmptyCodeHash.Bytes(),
			}
			val, _ := rlp.EncodeToBytes(acc)
...
func testGenerateFromEmptySnap(t *testing.T, scheme string) {
	...
	for i := 0; i < 400; i++ {
		stRoot := helper.makeStorageTrie(
			hashData([]byte(fmt.Sprintf("acc-%d", i))), []string{"key-1", "key-2", "key-3"},
...
func testGenerateWithIncompleteStorage(t *testing.T, scheme string) {
	...
	for i := 0; i < 8; i++ {
		...
		helper.addAccount(
			accKey,
			&types.StateAccount{Balance: uint256.NewInt(uint64(i)), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
		)
...
func populateDangling(disk ethdb.KeyValueStore) {
	...
	populate(
		...
		[]string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"},
	)
...
func testGenerateCompleteSnapshotWithDanglingStorage(t *testing.T, scheme string) {
	...
	helper.addAccount(
		"acc-3", &types.StateAccount{Balance: uint256.NewInt(1), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()},
	)

...
func TestFeeHistory(t *testing.T) {
	...
	for i, c := range cases {
		...
		first, reward, baseFee, ratio, blobBaseFee, blobRatio, err := oracle.FeeHistory(
			context.Background(), c.count, c.last, c.percent,
		)
		backend.teardown()
		...
		if len(blobRatio) != c.expCount {
			t.Fatalf(
				"Test case %d: blobGasUsedRatio array length mismatch, want %d, got %d", i, c.expCount, len(blobRatio),
			)
		}
		if len(blobBaseFee) != len(baseFee) {
			t.Fatalf(
				"Test case %d: blobBaseFee array length mismatch, want %d, got %d", i, len(baseFee), len(blobBaseFee),
			)
		}

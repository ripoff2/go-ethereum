	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				doTxNotify{
					peer: "A", hashes: []common.Hash{{0x01}, {0x02}},
					types: []byte{types.LegacyTxType, types.LegacyTxType}, sizes: []uint32{111, 222},
				},
				isWaiting(
					map[string][]announce{
						"A": {
				...
				doTxNotify{
					peer: "B", hashes: []common.Hash{{0x03}, {0x04}},
					types: []byte{types.LegacyTxType, types.LegacyTxType}, sizes: []uint32{333, 444},
				},
				isWaiting(
					map[string][]announce{
						"A": {
				...
				doTxNotify{
					...
					types: []byte{types.LegacyTxType, types.LegacyTxType}, sizes: []uint32{111, 444},
				},
				isWaiting(
					map[string][]announce{
						"A": {
				...
				doTxNotify{
					peer: "D", hashes: []common.Hash{{0x01}, {0x02}},
					types: []byte{types.LegacyTxType, types.BlobTxType}, sizes: []uint32{999, 222},
				},
				isWaiting(
					map[string][]announce{
						"A": {
				...
				isScheduled{
					...
					fetching: map[string][]common.Hash{
						// Depends on deterministic test randomizer
						"A": {{0x03}, {0x05}},
						"C": {{0x01}, {0x04}},
						"D": {{0x02}},
					},
				...
				doTxNotify{
					peer: "C", hashes: []common.Hash{{0x06}, {0x07}},
					types: []byte{types.LegacyTxType, types.LegacyTxType}, sizes: []uint32{666, 777},
				},
				isWaiting(
					map[string][]announce{
						"C": {
				...
				doTxNotify{
					peer: "E", hashes: []common.Hash{{0x06}, {0x07}},
					types: []byte{types.LegacyTxType, types.LegacyTxType}, sizes: []uint32{666, 777},
				},
				isScheduled{
					tracking: map[string][]announce{
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				doTxNotify{
					peer: "A", hashes: []common.Hash{{0x02}, {0x03}},
					types: []byte{types.LegacyTxType, types.LegacyTxType}, sizes: []uint32{222, 333},
				},
				isWaiting(
					map[string][]announce{
						"A": {
				...
				doTxNotify{
					peer: "B", hashes: []common.Hash{{0x02}, {0x03}, {0x04}},
					types: []byte{types.LegacyTxType, types.LegacyTxType, types.LegacyTxType},
					sizes: []uint32{222, 333, 444},
				},
				isWaiting(
					map[string][]announce{
						"A": {
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				doTxNotify{
					peer: "A", hashes: []common.Hash{{0x03}, {0x04}},
					types: []byte{types.LegacyTxType, types.LegacyTxType}, sizes: []uint32{333, 444},
				},
				isWaiting(
					map[string][]announce{
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				// data source.
				doTxNotify{
					peer: "B", hashes: []common.Hash{{0x02}}, types: []byte{types.LegacyTxType}, sizes: []uint32{222},
				},
				isWaiting(nil),
				isScheduled{
					tracking: map[string][]announce{
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				// Push an initial announcement through to the scheduled stage
				doTxNotify{
					peer: "A", hashes: []common.Hash{testTxsHashes[0]}, types: []byte{testTxs[0].Type()},
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				doTxEnqueue{
					peer: "A", txs: []*types.Transaction{testTxs[0]}, direct: true,
				}, // This depends on the deterministic random
				isScheduled{
					tracking: map[string][]announce{
						"A": {
							{testTxsHashes[2], testTxs[2].Type(), uint32(testTxs[2].Size())},
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				// should be dropped and the one after re-requested.
				doTxEnqueue{
					peer: "A", txs: []*types.Transaction{testTxs[1]}, direct: true,
				}, // This depends on the deterministic random
				isScheduled{nil, nil, nil},
			},
		},
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				doWait{time: txArriveTimeout, step: true},
				doTxNotify{
					peer: "A", hashes: []common.Hash{testTxsHashes[1]}, types: []byte{testTxs[1].Type()},
					sizes: []uint32{uint32(testTxs[1].Size())},
				},
				doWait{time: txArriveTimeout, step: true},
				doTxNotify{
					peer: "A", hashes: []common.Hash{testTxsHashes[2]}, types: []byte{testTxs[2].Type()},
					sizes: []uint32{uint32(testTxs[2].Size())},
				},
...
func TestTransactionFetcherWaitTimerResets(t *testing.T) {
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				doTxNotify{
					peer: "A", hashes: []common.Hash{{0x01}}, types: []byte{types.LegacyTxType}, sizes: []uint32{111},
				},
				isWaiting(
					map[string][]announce{
						"A": {
				...
				doTxNotify{
					peer: "A", hashes: []common.Hash{{0x02}}, types: []byte{types.LegacyTxType}, sizes: []uint32{222},
				},
				isWaiting(
					map[string][]announce{
						"A": {
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				doTxNotify{
					peer: "A", hashes: hashesA[:maxTxAnnounces/2], types: typesA[:maxTxAnnounces/2],
					sizes: sizesA[:maxTxAnnounces/2],
				},
				doTxNotify{
					peer: "B", hashes: hashesB[:maxTxAnnounces/2-1], types: typesB[:maxTxAnnounces/2-1],
					sizes: sizesB[:maxTxAnnounces/2-1],
				},
				doWait{time: txArriveTimeout, step: true},

				// Announce the second half and keep them in the wait list
				doTxNotify{
					peer: "A", hashes: hashesA[maxTxAnnounces/2 : maxTxAnnounces],
					types: typesA[maxTxAnnounces/2 : maxTxAnnounces], sizes: sizesA[maxTxAnnounces/2 : maxTxAnnounces],
				},
				doTxNotify{
					peer: "B", hashes: hashesB[maxTxAnnounces/2-1 : maxTxAnnounces-1],
					types: typesB[maxTxAnnounces/2-1 : maxTxAnnounces-1],
				...
				isScheduled{
					...
					fetching: map[string][]common.Hash{
						...
						"B": append(
							append([]common.Hash{}, hashesB[maxTxAnnounces/2-3:maxTxAnnounces/2-1]...),
							hashesB[:maxTxRetrievals-2]...,
						),
					},
				},
				// Ensure that adding even one more hash results in dropping the hash
				doTxNotify{
					peer: "A", hashes: []common.Hash{hashesA[maxTxAnnounces]}, types: []byte{typesA[maxTxAnnounces]},
					sizes: []uint32{sizesA[maxTxAnnounces]},
				},
				doTxNotify{
					peer: "B", hashes: hashesB[maxTxAnnounces-1 : maxTxAnnounces+1],
					types: typesB[maxTxAnnounces-1 : maxTxAnnounces+1],
					sizes: sizesB[maxTxAnnounces-1 : maxTxAnnounces+1],
				},
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				doTxEnqueue{
					peer: "B", txs: []*types.Transaction{testTxs[0], testTxs[1], testTxs[2], testTxs[3]}, direct: true,
				},
				isWaiting(nil),
				isScheduled{
					tracking: nil,
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				doWait{time: txArriveTimeout, step: true},
				doTxNotify{
					peer: "A", hashes: []common.Hash{{0x03}}, types: []byte{types.LegacyTxType}, sizes: []uint32{333},
				},
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				doWait{time: txArriveTimeout, step: true},
				doTxNotify{
					peer: "B", hashes: []common.Hash{{0x01}}, types: []byte{types.LegacyTxType}, sizes: []uint32{111},
				},
	...
	testTransactionFetcherParallel(
		t, txFetcherTest{
			...
			steps: []interface{}{
				...
				doTxNotify{
					peer: "B", hashes: []common.Hash{testTxsHashes[0]}, types: []byte{testTxs[0].Type()},
					sizes: []uint32{uint32(testTxs[0].Size())},
				},
				doWait{time: txArriveTimeout, step: true},
				doDrop("A"),
				...
				for _, ann := range announces {
					if meta, ok := waiting[ann.hash]; !ok {
						...
						if meta.kind != ann.kind || meta.size != ann.size {
							t.Errorf(
								"step %d, peer %s, hash %x: waitslot metadata mismatch: want %v, have %v/%v", i, peer,
								ann.hash, meta, ann.kind, ann.size,
							)
						}
					}
				}
			...
			for peer, announces := range step.tracking {
				...
				for _, ann := range announces {
					if meta, ok := scheduled[ann.hash]; !ok {
						...
						if meta.kind != ann.kind || meta.size != ann.size {
							t.Errorf(
								"step %d, peer %s, hash %x: announce metadata mismatch: want %v, have %v/%v", i, peer,
								ann.hash, meta, ann.kind, ann.size,
							)
						}
					}
